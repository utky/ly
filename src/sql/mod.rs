...
impl priority::Fetch for Session {
  fn fetch_priority_by_name(&mut self, name: &str) -> Result<priority::Priority> {
    self.conn.query_row_and_then("SELECT id, name, created_at, updated_at FROM priorities WHERE name = ?", params![name], |row| {
...
impl task::Add for Session {
  fn add_task(&mut self, lane_id: Id, priority: Id, summary: &str) -> Result<()> {
    self.conn.execute("INSERT INTO tasks(lane_id, priority, summary) VALUES (?, ?, ?)", params![lane_id, priority, summary])?;
    Ok(())
  }
}
...
impl task::Fetch for Session {
  fn fetch_task_by_id(&mut self, id: Id) -> Result<Option<task::Task>> {
    self.conn.query_row_and_then("SELECT id, lane_id, priority, summary, created_at, updated_at FROM tasks WHERE id = ?", params![id], |row| {
      let t = row_to_task(row)?;
      Ok(Some(t))
    })
  }
  fn fetch_all_tasks(&mut self, lane_name: &str) -> Result<Vec<task::Task>> {
    let mut stmt = self.conn.prepare("SELECT id, lane_id, priority, summary, created_at, updated_at FROM tasks WHERE EXISTS (SELECT id FROM lanes WHERE name = ? AND lanes.id = tasks.lane_id) ORDER BY priority DESC")?;
